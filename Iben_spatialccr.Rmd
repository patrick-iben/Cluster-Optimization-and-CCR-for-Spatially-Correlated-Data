...
### MAPPING PACKAGES ###
#install.packages(c("RColorBrewer", "sp", "maptools", "lattice", "latticeExtra", "rgdal","classInt", "gridExtra","ggplot2","tidyr", "cluster", "ClustGeo", "WeightedCluster", "cvcrand", "spdep", "foreign", "spatstat", "gmodels", "tools"))
...
#If ((closest even number under number of units divided by 3) *3)) is greater than number of units, then subtract 2 from previous line for the maximum cluster size
maxclust <- ifelse((n_3_even*3) > nrow(study_data_wide), n_3_even-2, n_3_even)
#For this example, 18 clusters maximum will be tested
```
...
#Initializing listing of cluster value quality for each value of K
quality_list <- list()

#Average Silhouette Width (observation)
#Initializing cluster quality statistic matrix
...
#Only optimizing for even K values for CCR (starting from minimum K)
ASW_valid <- ASW[seq(1, nrow(ASW), 2),]
...
#Saving cluster selection run time
...
#Getting aggregate statistics by cluster
...
#Labeling rows
rownames(Kmeans_clusters_weighted_ccr_1) <- Kmeans_clusters_weighted_ccr_1[,2]

#Balance score for min. scheme
weighted_kmeans_ccr_min_score <- as.numeric(kmeans_Weighted_Design_result$bscores[5,2])
#Balance score for selected scheme
weighted_kmeans_ccr_selected_score <- as.numeric(kmeans_Weighted_Design_result$bscores[1,2])
#Balance score for 10% cutoff scheme
weighted_kmeans_ccr_cutoff_score <- as.numeric(kmeans_Weighted_Design_result$bscores[2,2])
#Balance score for max. scheme
weighted_kmeans_ccr_max_score <- as.numeric(kmeans_Weighted_Design_result$bscores[14,2])
...
#Labeling rows
rownames(Kmeans_clusters_unweighted_ccr_1) <- Kmeans_clusters_unweighted_ccr_1[,2]

#Balance score for min. scheme
unweighted_kmeans_ccr_min_score <- as.numeric(kmeans_Unweighted_Design_result$bscores[5,2])
#Balance score for selected scheme
unweighted_kmeans_ccr_selected_score <- as.numeric(kmeans_Unweighted_Design_result$bscores[1,2])
#Balance score for 10% cutoff scheme
unweighted_kmeans_ccr_cutoff_score <- as.numeric(kmeans_Unweighted_Design_result$bscores[2,2])
#Balance score for max. scheme
unweighted_kmeans_ccr_max_score <- as.numeric(kmeans_Unweighted_Design_result$bscores[14,2])
...
#Only looking at even K values for CCR (starting from minimum K)
ASW_valid <- ASW[seq(1, nrow(ASW), 2),]
...
#Labeling rows
rownames(clara_clusters_weighted_ccr_1) <- clara_clusters_weighted_ccr_1[,2]

#Balance score for min. scheme
weighted_clara_ccr_min_score <- as.numeric(CLARA_Weighted_Design_result$bscores[5,2])
#Balance score for selected scheme
weighted_clara_ccr_selected_score <- as.numeric(CLARA_Weighted_Design_result$bscores[1,2])
#Balance score for cutoff scheme
weighted_clara_ccr_cutoff_score <- as.numeric(CLARA_Weighted_Design_result$bscores[2,2])
#Balance score for max. scheme
weighted_clara_ccr_max_score <- as.numeric(CLARA_Weighted_Design_result$bscores[14,2])
...
#Labeling rows
rownames(clara_clusters_unweighted_ccr_1) <- clara_clusters_unweighted_ccr_1[,2]

#Balance score for min. scheme
unweighted_clara_ccr_min_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[5,2])
#Balance score for selected scheme
unweighted_clara_ccr_selected_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[1,2])
#Balance score for cutoff scheme
unweighted_clara_ccr_cutoff_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[2,2])
#Balance score for max. scheme
unweighted_clara_ccr_max_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[14,2])
...
#Borders water indicator (ensuring factorization prior to spatial plotting)
...
sp_out_hhDensity <- spplot(map, c("hhDensity"), 
       colorkey=list(space="right"), scales = list(draw = TRUE), 
       main = "Village Household Density (per square kilometer)", as.table = TRUE, 
        col = "transparent", cuts = 8)
sp_out_hhDensity
```
...
```{r, echo=T, message=F, warning=F}
#Most recent survey positive microscopy rate
sp_out_mrate5 <-spplot(map, c("mrate_5"), 
       names.attr = c("Survey 5"), 
       colorkey=list(space="right"), scales = list(draw = TRUE), 
       main = "Malaria Prevalence Rate, Survey 5", as.table = TRUE, 
       col.regions = tempPal, col = "transparent", at = seq(0, 0.5, 0.1))
sp_out_mrate5
```

```{r, echo=F, message=F, warning=F}
#Microscopy rate, each survey
...
#Setting max. cluster size for "ClustGeo"
...
#Setting values of alpha to optimize over
...
#Minimizing difference between Q0 and Q1
Q0_Q1_diff <- abs(Q0-Q1)

#Only looking at even K values for CCR (starting from minimum K) 
Q0_Q1_diff_valid <- Q0_Q1_diff[seq(1, nrow(Q0_Q1_diff), 2), ] 

#Selecting optimal K as minimized magnitude difference between Q0 and Q1
...
#Merging back hierarchical clustering scheme
study_data_wide <- merge(PKbis_mat, study_data_wide, by="village_code")
#Renaming to match SpatialPolygonsDataframe
colnames(PKbis_mat) <- c("Hierarch_Clusters", "village_sc")
...
#Labeling rows
rownames(hclust_clusters_weighted_ccr_1) <- hclust_clusters_weighted_ccr_1[,2]

#Balance score for min. scheme
weighted_hclust_ccr_min_score <- as.numeric(hclust_Weighted_Design_result$bscores[5,2])
#Balance score for selected scheme
weighted_hclust_ccr_selected_score <- as.numeric(hclust_Weighted_Design_result$bscores[1,2])
#Balance score for cutoff scheme
weighted_hclust_ccr_cutoff_score <- as.numeric(hclust_Weighted_Design_result$bscores[2,2])
#Balance score for max. scheme
weighted_hclust_ccr_max_score <- as.numeric(hclust_Weighted_Design_result$bscores[14,2])
...
#Labeling rows
rownames(hclust_clusters_unweighted_ccr_1) <- hclust_clusters_unweighted_ccr_1[,2]

#Balance score for min. scheme
unweighted_hclust_ccr_min_score <- as.numeric(hclust_Unweighted_Design_result$bscores[5,2])
#Balance score for selected scheme
unweighted_hclust_ccr_selected_score <- as.numeric(hclust_Unweighted_Design_result$bscores[1,2])
#Balance score for cutoff scheme
unweighted_hclust_ccr_cutoff_score <- as.numeric(hclust_Unweighted_Design_result$bscores[2,2])
#Balance score for max. scheme
unweighted_hclust_ccr_max_score <- as.numeric(hclust_Unweighted_Design_result$bscores[14,2])
...
text(coordinates(map), labels = PKbis_geo)
```
...
#Labeling rows
rownames(hclust_geo_clusters_weighted_ccr_1) <- hclust_geo_clusters_weighted_ccr_1[,2]

#Balance score for min. scheme
weighted_hclust_geo_ccr_min_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[5,2])
#Balance score for selected scheme
weighted_hclust_geo_ccr_selected_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[1,2])
#Balance score for cutoff scheme
weighted_hclust_geo_ccr_cutoff_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[2,2])
#Balance score for max. scheme
weighted_hclust_geo_ccr_max_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[14,2])
...
#Labeling rows
rownames(hclust_geo_clusters_unweighted_ccr_1) <- hclust_geo_clusters_unweighted_ccr_1[,2]

#Balance score for min. scheme
unweighted_hclust_geo_ccr_min_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[5,2])
#Balance score for selected scheme
unweighted_hclust_geo_ccr_selected_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[1,2])
#Balance score for cutoff scheme
unweighted_hclust_geo_ccr_cutoff_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[2,2])
#Balance score for max. scheme
unweighted_hclust_geo_ccr_max_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[14,2])
...
#Saving CIs for mean nearest-neighbor distance, by treatment arm
#Treatment 0
...
#Testing for significant differences in treatment arm mean nearest-neighbor distance with Welch Two Sample t-test
...
#Hierarchical - geographic location only
hclust_geo_ccr_scores <- cbind(c(weighted_hclust_geo_ccr_min_score,unweighted_hclust_geo_ccr_min_score), c(weighted_hclust_geo_ccr_selected_score,unweighted_hclust_geo_ccr_selected_score), c(weighted_hclust_geo_ccr_cutoff_score, unweighted_hclust_geo_ccr_cutoff_score), c(weighted_hclust_geo_ccr_max_score,unweighted_hclust_geo_ccr_max_score), c(K_hclust_geo/2, K_hclust_geo/2), c(as.numeric(levels(hclust_geo_Weighted_Design_result$overall_allocations$`accepted allocations`)), as.numeric(levels(hclust_geo_Unweighted_Design_result$overall_allocations$`accepted allocations`))))
...
#Ward-like Hierarchical with mixed dissimilarity
...
In terms of selected balance score, the top three performers for weighted CCR were: CLARA (9.297); Ward-like hierarchical clustering, using geographic location (15.198); and K-Means (26.295). The top three performers for unweighted CCR were: CLARA (0.776); Ward-like hierarchical clustering, using geographic location (1.632); and Ward-like hierarchical clustering, using mixed dissimilarities (1.886). In terms of the maximum (10%) cutoff score, the top three performers for weighted CCR were: CLARA (11.158); Ward-like hierarchical clustering, using geographic location (18.469); and K-Means (29.857). The top three performers for unweighted CCR were: CLARA (1.767); Ward-like hierarchical clustering, using geographic location (2.002); and Ward-like hierarchical clustering, using mixed dissimilarities (1.886).
...
In terms of minimizing covariate imbalance, Ward-like hierarchical using only geographic location (Euclidean distance) and weighting during CCR appears to perform better than the more complicated, and more computationally-intensive, mixed-dissimilarity method. For CRT design using Ward-like hierarchical clustering with weighted CCR, this added level of sophistication may be unnecessary. Clustering on geographic location, then performing CCR, may sufficiently balance treatment arms with respect to important spatial feature data. For some studies, if a straight-line path is not possible (e.g., a grid of city streets), it may be impractical to use Euclidean distance between units. For such a scenario, using Manhattan distance is preferable; however, it should be noted that K-means gives better results (i.e., less distortion) when using Euclidean distance [12].

For this data scenario, for both weighted and unweighted CCR, based on 10% cutoff balance scores and CPU run times, either CLARA or Ward-like hierarchical clustering with Euclidean geographical dissimilarities may be considered optimal study design strategies for ensuring covariates are roughly balanced and units within a cluster are spatially contiguous. For unweighted CCR, both selected schemes for CLARA and hierarchical maintained spatial heterogeneity of treatment assignments. CLARA had the lowest cutoff score; thus, for unweighted CCR, CLARA performed the best. For weighted CCR, only the selected scheme for hierarchical also maintained spatial heterogeneity of treatment assignments. Thus, for weighted CCR, Ward-like hierarchical clustering with Euclidean geographical dissimilarities performed the best. The cluster and treatment assignments for this method can be seen in Figure 4b. For comparison, results from a treatment allocation without spatial heterogeneity of treatment arms - Ward-like hierarchical clustering with mixed dissimilarities and weighted CCR - are provided in Figure 4d. We view 4b as a better randomization as 4d because the covariates are roughly balanced and the shapes are contiguous within arms. We view 4d as introducing a higher potential for intervention mixing. 

In this paper, we presented a stepwise methodological approach to mitigating covariate imbalance and intervention contamination in a CRT. This approach cannot ensure that only treatment assignments exhibiting spatial heterogeneity are randomly selected from. Future enhancements to this approach might include optimization of covariate imbalance and spatial clustering of intervention arms simultaneously.

###Limitations

For simplicity, the only cluster quality statistic used to optimize clustering methods was average silhouette width. It may be of interest to examine additional, potential cluster quality statistics that are applicable for clustering on a variety of data types (e.g., numerical, nominal, ordinal). Furthermore, "ClustGeo" limits the maximum number of clusters to 55; this may be impractical for some large CRT studies. For large CRTs, K-means or CLARA - or an alternate method of hierarchical clustering - may be preferable to Ward-Like hierarchical clustering with "ClustGeo". 
...
Unlike the spatial clustering methods outlined here, density-based spatial clustering methods (e.g., DBSCAN) can detect clusters of arbitrary shape [13]. As the geographic region for this and many studies is oblong, and not uniformly- distributed, such an algorithm may be of use for assigning units to CRT treatment arms. Unfortunately, as it is not a hard clustering algorithm and may result in some units belonging to multiple or no clusters, density-based clustering was not considered in this example.
...
12. Singh, A., Yadav, A., & Rana, A. (2013). K-means with Three different Distance Metrics. International Journal of Computer Applications, 67(10), 13-17. doi: 10.5120/11430-6785

13. Ram A, Jalal S, Jalal AS, Kumar M. (2010). A Density Based Algorithm for Discovering Density Varied Clusters in Large Spatial Databases. International Journal of Computer Applications. 3(6):1-4.
