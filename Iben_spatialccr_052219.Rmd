...
To measure covariate and geographical dissimilarity between units, and to accommodate mixed data types, the Gower metric is used via the daisy() function in the "cluster" package. For K-means and CLARA, optimal clustering number is determined with the wcClusterQuality() function from the "WeightedCluster" package. For Hierarchical, optimal clustering number is determined with the "ClustGeo" package.
...
By considering sub-datasets of a fixed size, CLARA can handle much larger datasets than other partitioning methods like k-means [9]. To perform this, we use the clara() function from the "cluster" package.
...
#Balance score for min scheme
weighted_clara_ccr_min_score <- as.numeric(CLARA_Weighted_Design_result$bscores[5,2])#0.028
#Balance score for selected scheme
weighted_clara_ccr_selected_score <- as.numeric(CLARA_Weighted_Design_result$bscores[1,2])#0.111
#Balance score for cutoff scheme
weighted_clara_ccr_cutoff_score <- as.numeric(CLARA_Weighted_Design_result$bscores[2,2])#0.15
#Balance score for max scheme
weighted_clara_ccr_max_score <- as.numeric(CLARA_Weighted_Design_result$bscores[14,2])#1.756
...
#Balance score for min scheme
unweighted_clara_ccr_min_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[5,2])#0.223
#Balance score for selected scheme
unweighted_clara_ccr_selected_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[1,2])#1.785
#Balance score for cutoff scheme
unweighted_clara_ccr_cutoff_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[2,2])#1.86
#Balance score for max scheme
unweighted_clara_ccr_max_score <- as.numeric(CLARA_Unweighted_Design_result$bscores[14,2])#13.182
```
...
Next, using the daisy() function from the "cluster" package, we create dissimilarity matrices for the covariates and geographical location variables separately. In addition to accommodating non-Euclidean dissimilarities, the "Clustgeo" package allows for weighting of spatial feature and geographical dissimilarity importance during cluster assignment. To test the varying weightings of dissimilarity matrices for our mixed data types, Gower dissimilarity is utilized for spatial feature data using the daisy() function - with metric set to "gower". Briefly, Gower dissimilarity uses the Dice coefficient for binary data and range-normalized Manhattan distance for numeric data [10].
...
#Plotting
sp::plot(map,border="grey",col=PKbis, main= "Ward-Like Hierarchical Clustering of Size 18 Obtained with alpha=0.7",cex.main=1)
legend("topleft", legend=paste("cluster",1:max(PKbis)), fill=1:max(PKbis), bty="n",border="grey")

#Black and white
#plot(map, main="Ward-Like Hierarchical Clustering of Size 18 Obtained with alpha=0.7",cex.main=1 )
...
#Balance score for selected scheme
weighted_hclust_ccr_selected_score <- as.numeric(hclust_Weighted_Design_result$bscores[1,2])#0.263
#Balance score for cutoff scheme
weighted_hclust_ccr_cutoff_score <- as.numeric(hclust_Weighted_Design_result$bscores[2,2])#0.358
#Balance score for max. scheme
weighted_hclust_ccr_max_score <- as.numeric(hclust_Weighted_Design_result$bscores[14,2])#11.742
```
...
#Balance score for min. scheme
unweighted_hclust_ccr_min_score <- as.numeric(hclust_Unweighted_Design_result$bscores[5,2])#0.005
#Balance score for selected scheme
unweighted_hclust_ccr_selected_score <- as.numeric(hclust_Unweighted_Design_result$bscores[1,2])#2.274
#Balance score for cutoff scheme
unweighted_hclust_ccr_cutoff_score <- as.numeric(hclust_Unweighted_Design_result$bscores[2,2])#3.546
#Balance score for max. scheme
unweighted_hclust_ccr_max_score <- as.numeric(hclust_Unweighted_Design_result$bscores[14,2])#71.181
...
#Black and white
#plot(map, main="Ward-Like Hierarchical Clustering of Size 12 Obtained with only Geographic Location \n (alpha = 1)",cex.main=1 )
...
#Balance score for min. scheme
weighted_hclust_geo_ccr_min_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[5,2])#0.01
#Balance score for selected scheme
weighted_hclust_geo_ccr_selected_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[1,2])#0.184
#Balance score for cutoff scheme
weighted_hclust_geo_ccr_cutoff_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[2,2])#0.26
#Balance score for max. scheme
weighted_hclust_geo_ccr_max_score <- as.numeric(hclust_geo_Weighted_Design_result$bscores[14,2])#4.486
...
#Balance score for min. scheme
unweighted_hclust_geo_ccr_min_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[5,2])#0.19
#Balance score for selected scheme
unweighted_hclust_geo_ccr_selected_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[1,2])#1.378
#Balance score for cutoff scheme
unweighted_hclust_geo_ccr_cutoff_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[2,2])#1.816
#Balance score for max. scheme
unweighted_hclust_geo_ccr_max_score <- as.numeric(hclust_geo_Unweighted_Design_result$bscores[14,2])#37.997
```
...
In terms of selected balance score, the top four performers were: CLARA with weighted CCR (0.111); Ward-Like Hierarchical clustering with weighted CCR, using geographic location (0.145); K-Means with weighted CCR (0.248); Ward-Like Hierarchical clustering with weighted CCR, using geographic location and covariate dissimilarity (0.263). In terms of the maximum (10%) cutoff score, the top four performers were: CLARA with weighted CCR (0.150); Ward-Like Hierarchical clustering with weighted CCR, using geographic location (0.178); K-Means with weighted CCR (0.294); Ward-Like Hierarchical clustering with weighted CCR, using geographic location and covariate dissimilarity (0.358). Ward-Like Hierarchical clustering with weighted CCR, using geographic location and covariate dissimilarity, had the lowest minimum score in their randomization space (0.000), while Ward-Like Hierarchical clustering with weighted CCR, using geographic location and covariate dissimilarity, had the highest maximum score in their randomization space (71.181); it should be noted that both of these CCR tests had the highest number of clusters in each treatment arm (9), and thus the highest number of possible treatment allocations in their randomization space (4862).
...
At significance level alpha = 0.05, the CCR allocations that resulted in significantly different mean nearest neighbor distances between treatment arms were K-Means with weighted CCR, CLARA with unweighted CCR, Ward-Like Hierarchical with weighted CCR (using Geographic location) and Ward-Like Hierarchical with weighted CCR (using location and covariate dissimilarity).
...
In terms of CPU run times, the quickest performer was Hierarchical using geographic location, followed by K-Means and CLARA (no discernible differences between them). Ward-Like Hierarchical clustering with mixed dissimilarities took the longest to run, due to its cluster optimization phase, though by only a few extra seconds. 
...
Strategic weighting of covariates during CCR, regardless of clustering method, may serve to drastically improve covariate balance during randomization into CRT treatment arms.

In terms of minimizing covariate imbalance, Ward-Like Hierarchical using only geographic location (Euclidean distance) and weighting during CCR appears to perform better than the more complicated, and more compuationally-intensive, mixed-dissimilarity method. For CRT design using Ward-Like Hierarchical clustering with weighted CCR, this added level of sophistication may be unnecessary.

Based on 10% cutoff balance scores and CPU run times, either CLARA with weighted CCR or Ward-Like Hierarchical Clustering with Euclidean geographical dissimilarities and weighted CCR may be considered optimal study design strategies. Only the selected scheme for hierarchical also maintained spatial heterogeneity of treatment assignments for both studies; thus, for this example, Ward-Like Hierarchical Clustering with Euclidean geographical dissimilarities and weighted CCR performed the best. 
...
It appears as if minimum and maximum CCR scores may be influenced by number of clusters to be randomized into treatment arms. It may be unwise to interpret these scores.
