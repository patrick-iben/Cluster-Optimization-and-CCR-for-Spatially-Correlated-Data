---
title: "Cluster Assignment and Stratified Covariate Constrained Randomization for Spatially-Correlated Data: Manhattan Distances"
author: "Patrick Iben"
date: "January 27, 2020"
output: html_document
---

This document provides a method on selecting clusters and treatment assignments for stratified cluster randomized trials, via covariate-constrained randomization and from spatially-correlated data. Three clustering techniques are used: K-Means; Clustering Large Applications (CLARA); Ward-Like Hierarchical Clustering. The distance metric is Manhattan distance. Optimal treatment assignments are detrmined by: balance on strata, evidence of spatial heterogeneity of treatment assignments 
...
dist_matrix <- as.matrix(daisy(geo_data, metric = "manhattan"))

#Saving nearest treatment arm neighbor (Manhattan distance)
nn_manhattan <- apply(dist_matrix, 1, FUN = function(x) {min(x[x > 0])})
```
...
for(i in seq(minclust,maxclust, by = 4)){
  set.seed(seed)
  clustering_list[[i-(minclust-1)]] <- Kmeans(geo_data, i, method = "manhattan")$cluster
}
...
#Merging nearest neighbor distance
trt_loc[[i]] <- cbind(trt_loc[[i]], nn_manhattan)
...
spat_clust[i,] <- ifelse(t.test(split_trt_loc[[i]]$`0`$nn_manhattan,split_trt_loc[[i]]$`1`$nn_manhattan)$p.value < 0.05, 1, 0) 
...
#Collecting cluster quality statistics across range of cluster sizes
for(i in seq(minclust,maxclust, by = 4)){
  clustering_list[[i-(minclust-1)]] <- clara(geo_data, metric = "manhattan", k = i)$cluster
}
...
#Merging with nearest neighbor (Manhattan)
trt_loc[[i]] <- cbind(trt_loc[[i]], nn_manhattan)
...
#Performing non-Euclidean Ward-Like hierarchical clustering with hclustgeo()
tree <- hclustgeo(as.dist(dist_matrix))
...
for(i in 1:length(clustering_list)){
  ...
  colnames(hclust_cluster_assignments) <- cat("hclust_",max(hclust_cluster_assignments[,i]), sep = "")
}
...
#Merging nearest neighbor (Manhattan)
trt_loc[[i]] <- cbind(trt_loc[[i]], nn_manhattan)
...
#Testing for balance on strata (regex used to find string)
...
#Saving design
write.csv(overall_best_design[[1]]$baseline_table, "overall_best_ccr_table_manhattan.csv")
write.csv(overall_best_design_df, "overall_best_ccr_assignments_manhattan.csv")
